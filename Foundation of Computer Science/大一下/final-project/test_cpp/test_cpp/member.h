...
void member::print_interest(unsigned rest, std::ofstream& os) {
    ...
    os.open("/Users/jeffrey/Desktop/test_cpp/test_cpp/data/" + name + "_" + number + ".txt", std::ios::app);
    os << record << '\n';
    os.close();
}
...
void member::print_record(void) {
    std::for_each(records.begin(), records.end(), [](const std::string& data){ std::cout << data << '\n'; });
    std::cout << "\n-------------------------------------------------------------------\n";
}
